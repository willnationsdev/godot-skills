...
func _apply(p_source, p_target_read, p_target_write, p_params = {}):
	var new_value = 0.0 if assign_directly else Skills.prop_get(p_target_read, property_name)
	var temp_max_value = Skills.prop_get(p_target_read, max_property_name) if max_property_name else null
	var temp_min_value = Skills.prop_get(p_target_read, min_property_name) if min_property_name else null
	...
	Skills.prop_set(p_target_write, property_name, _convert_value(new_value))